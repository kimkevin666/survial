...
public class NPC : MonoBehaviour
{
    ...
    [Header("Cambat")] public int damage;
...
// Start is called before the first frame update
    ...
    void PassiveUpdate()
    {
        if (aiState == AIState.Wandering && agent.remainingDistance < 0.1f)
        {
            ...
            Invoke("WanderToNewLocation", Random.Range(minWanderWaitTime, maxWanderWaitTime)); // " "색깔?
        }
    ...
    void AttackingUpdate()
    {
        if (playerDistance < attackDistance && IsPlayerInFieldOfView())
        {
            ...
            if (Time.time - lastAttackTime > attackRate)
            {
                ...
                CharacterManager.Instance.Player.controller.GetComponent<IDamagaIbe>().TakePhysiaclDamage(damage);
            }
        }
        else
        {
